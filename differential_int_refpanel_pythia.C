...
const char* Hydjet_QG = "HydjetDrum_Pyquen_Dijet80_FOREST_Track8_Jet24_FixedPtHat_v0_mergedpkurt_0_CFMSKIM_20140403_QG.root";
...
{
  TFile * ppfile = new TFile("plots/Pythia80_Aj01_newtest.root","read");
  ...
  niceTH1(dummy_p, 40, -40, 505, 406);

  for(Int_t iter = 0; iter < 10; iter++){
    dummy_p->SetBinContent(iter + 1, 40);
  }
  ...
  for(Int_t histIter = 0; histIter < 6; histIter++){
    hist1_p[histIter] = new TH1F(Form("hist1%s_%s_p", namePT[histIter],gorr), Form("hist1%s_%s_p", namePT[histIter],gorr), 10, binArrayX);
    hist2_p[histIter] = new TH1F(Form("hist2%s_%s_p", namePT[histIter],gorr), Form("hist2%s_%s_p", namePT[histIter],gorr), 10, binArrayX);
    ...
    if(strcmp(gorr,"g") == 0) getGraph_pp_p[histIter] = (TGraphErrors*)ppfile->Get(Form("gTImbDelR%s%s%s_%s_Pythia80_QG_g", perpProj, namePT[histIter], Corr, GLN));
    if(strcmp(gorr,"r") == 0) getGraph_pp_p[histIter] = (TGraphErrors*)ppfile->Get(Form("rVsCaloImbDelR%s%s%s_%s_Pythia80_QG_g", perpProj, namePT[histIter], Corr, GLN));
  }
...
//in mode for 0.5-1 GeV particles at the moment

  hist1_p[5]->Add(hist_pp_p[5],-1);
  hist1_p[5]->GetYaxis()->SetTitle("PbPb-pp");
  hist1_p[5]->GetYaxis()->SetRangeUser(-40,40);
  hist1_p[5]->DrawCopy("E1");
  //hist1_p[5]->SetName(Form("%s%s%s_peri",gorr,algType[setNum],Corr));
  //hist1_p[5]->Write(Form("%s%s%s_peri",gorr,algType[setNum],Corr));
  hist1_p[5]->Write();  
  ...
  hist2_p[5]->GetYaxis()->SetRangeUser(-40,40);
  hist2_p[5]->DrawCopy("E1");
  //hist2_p[5]->SetName(Form("%s%s%s_cent",gorr,algType[setNum],Corr));
  //hist2_p[5]->Write(Form("%s%s%s_cent",gorr,algType[setNum],Corr));
...
void cfmDiJet_DelRPlots_int(const char* inName, const char* outName, Bool_t montecarlo = false)
{
  ...
  else if(!strcmp(inName, Hydjet_QG)){
    std::cout << Hydjet_QG << std::endl;
    fileTag1 = "Hydjet_QG";
  }
...
//set algiter to 4 if running only over gen
